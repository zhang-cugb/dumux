...
public:
    ...
    {
        const auto vgn = getParam<Scalar>(paramGroup + ".Vgn");
        const auto vgm = 1.0 - 1.0/vgn;
        const auto vgAlpha = getParam<Scalar>(paramGroup + ".VgAlpha");
        return {vgn, vgm, vgAlpha};
    }
    ...
    static Scalar krn(Scalar swe, const Params<Scalar>& params)
    {
        ...
        return pow(1 - swe, 1.0/3) * pow(1 - pow(swe, 1.0/params.m), 2*params.m);
    }
    ...
    static Scalar dkrn_dswe(Scalar swe, const Params<Scalar>& params)
    {
        ...
        const Scalar x = pow(swe, 1.0/params.m);
        return -pow(1.0 - x, 2*params.m) * pow(1.0 - swe, -2.0/3) * (1.0/3 + 2*x/swe);
    }
};
